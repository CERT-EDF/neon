...
_LOGGER = get_logger('analyzer.capa', root='neon')


@dataclass(kw_only=True)
class CapaAnalyzerConfig(FusionAnalyzerConfig):
    """Capa Analyzer Config"""
...
async def _capa_process_impl(
    info: AnalyzerInfo,
    config: CapaAnalyzerConfig,
    storage: Storage,
    a_task: AnalyzerTask,
) -> bool:
...
def main():
    ...
    analyzer = Analyzer(
        ...
        config_cls=CapaAnalyzerConfig,
        process_impl=_capa_process_impl,
    )
    analyzer.run()
